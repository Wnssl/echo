...
class TimeConv(nn.Module):
    def __init__(self):
        ...
        # 使用一个卷积核为 2 的 3D 卷积，步幅为 2，输出通道数为 3
        self.conv3d = nn.Conv3d(
            in_channels=3,    # 输入通道数
            out_channels=3,   # 输出通道数
            kernel_size=(2, 1, 1),  # 时间方向上卷积核大小为 2，其余维度为 1
            stride=(2, 1, 1)  # 步幅为 2，以便每两帧计算一次
        )
...
class final_model(nn.Module):
    def __init__(self):
        super().__init__()
        self.tDownsample = TimeConv()
        self.base_model = unireplknet_t(num_classes=1)



    def forward(self, x):
        x = self.tDownsample(x)
...
class final_model2(nn.Module):
    def __init__(self, pretrain=True, num_class=1, output_dim=640, fds=True,bucket_num=80,bucket_start=20, 
                 start_update=20, start_smooth=21,kernel='gaussian',ks=5,sigma=1,momentum=0.9,dropout=0.2,
                 time_downsample=True, include_top=True,
                ):
        super().__init__()
        ...
        if self.fds:
            ...
            self.FDS = FDS(
                ...
                start_smooth=self.start_smooth,
                kernel=self.kernel,
                ks=self.ks,
                sigma=self.sigma,
        ...
        # model --- pretrain output_dim numclas
        
        
        model = unireplknet_t()
        # model = timm.create_model('unireplknet_s', num_classes=num_class, in_22k_pretrained=False)
        if self.pretrain:
            # checkpoint = torch.load('unireplknet_s_in22k_pretrain.pth')
            checkpoint = torch.load('unireplknet_t_in1k_224_acc83.21.pth')
            print('加载了参数')
        ...
        if self.include_top:    
            self.head = nn.Linear(self.output_dim, self.num_class)


    def forward_feature(self, x, targets=None, epoch=None):
        if self.time_downsample:
            x = self.tDownsample(x)
        bs, c, t, w, h = x.shape
        m = math.sqrt(t)
        x = x.view(bs, c, int(w *m), int(m * h))
        x = self.feature(x)
        return x
    
    def forward(self, x, targets=None, epoch=20):
        feature = self.forward_feature(x)
        # X_c x copy
        x_c = feature
        if self.training and self.fds:
            if epoch > self.start_smooth:
                x_c = self.FDS.smooth(x_c, targets, epoch)
...
if __name__ == '__main__':
    ...
    # print(y)
    # print(y.shape)
    model = timm.create_model('unireplknet_t', num_classes=1, in_1k_pretrained=True)
    print(timm.create_model('unireplknet_t').default_cfg)
