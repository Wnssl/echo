...
def train_one_epoch(model, regressor, device, optimizer, optimizer_regressor, criterion, scheduler, epoch, train_loader,
    ...
    for idx, batch in tqdm(enumerate(train_loader)):
        views1, views2 = batch
        images = torch.cat([views1[0], views2[0]], dim=0)
        labels = views1[1]
        bsz = labels.shape[0]
...
def main():
    ...
    parser.add_argument('--epoch_nums', type=int, help='epoch nums', default=60)
    ...
    parser.add_argument('--num_workers', type=int, help='cpu workers', default=10)
    ...
    if args.scheduler == 'StepLR':
        ...
        scheduler_reg = optim.lr_scheduler.StepLR(optimizer_reg, step_size=args.step_num, gamma=args.gamma)
    else:
        # 自定义scheduler
        scheduler = None
        pass
    ...
    for epoch in range(args.epoch_nums):
        ...
        print(f'当前最佳模型{best_epoch}  test : {best_result}')
